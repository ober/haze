...
def readInstanceTag(instanceID, tagName="Name", connection=None):
    ...
    :rtype: str
    """
    assert isinstance(instanceID, str), ("instanceID must be a string but is %r" % instanceID)
    assert isinstance(tagName, str), ("tagName must be a string but is %r" % tagName)
